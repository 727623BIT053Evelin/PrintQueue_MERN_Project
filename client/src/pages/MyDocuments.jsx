...
const MyDocuments = () => {
    ...
    // Razorpay payment success is handled in UploadDocument.jsx via the modal callback.
    // No special redirect handling is needed here unless you want to handle specific URL params.
    ...
    // Group jobs by batch and sort them
    const groupJobsByBatch = () => {
        ...
        jobs.forEach(job => {
            if (job.batchId) {
                ...
                // Keep the oldest createdAt for the batch to determine its base age, 
                // but we will use the newest for sorting if it's "New"
                if (new Date(job.createdAt) > new Date(batches[job.batchId].createdAt)) {
                    batches[job.batchId].createdAt = job.createdAt;
                }
                ...
                individualJobs.push({ ...job, isBatch: false });
            }
        });

        // Combine and sort
        const allItems = [...Object.values(batches), ...individualJobs];
        
        allItems.sort((a, b) => {
            // Priority 1: Pending & (Unpaid or Unconfirmed) - Actionable
            // Priority 2: Pending & (Paid & Confirmed) - In Progress
            // Priority 3: Completed / Collected - Done
            
            const getPriority = (item) => {
                const isPending = item.status === 'pending' || item.status === 'printing';
                const needsAction = item.isBatch 
                    ? (!item.allPaid || !item.allConfirmed) 
                    : (!item.isPaid || !item.confirmedPresence);
                
                if (isPending && needsAction) return 1;
    ...
    const sortedItems = groupJobsByBatch();
