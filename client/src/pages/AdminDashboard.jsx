...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        socket.on('jobUpdated', (data) => {
            console.log('📨 Received jobUpdated event:', data.type, data.job?._id || data.jobId);
            if (data.type === 'create') {
    ...
    useEffect(() => {
        console.log('🔄 Jobs state updated, count:', jobs.length);
        const printingJobs = jobs.filter(j => j.status === 'printing');
        if (printingJobs.length > 0) {
            console.log('🖨️ Currently printing jobs:', printingJobs.map(j => ({ 
                id: j._id, 
                printerId: j.printer?._id || j.printer,
                printerName: j.printer?.name || 'Unknown'
            })));
        } else {
            console.log('😴 No jobs currently printing');
        }
    }, [jobs]);
    ...
    const isPrinterBusy = (printerId) => {
        ...
        return jobs.some(job => {
            ...
            const jobPrinterId = (job.printer?._id || job.printer || '').toString();
            return jobPrinterId === targetId;
        });
    };

    // Get number of pending jobs for a specific printer
    const getPrinterQueueCount = (printerId) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex justify-between items-center mb-8 animate-slide-up">
                ...
                <div className="flex items-center space-x-4">
                    <button 
                        onClick={() => { fetchJobs(); fetchPrinters(); }}
                        className="glass-panel p-2 text-indigo-600 hover:bg-indigo-50 transition-all"
                        title="Refresh Data"
                    >
